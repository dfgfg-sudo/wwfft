...
class ZipProcessor {
    ...
    load(zipBuffer) {
        try {
            this.zip = new adm_zip_1.default(Buffer.from(zipBuffer));
            this.entries = [];
            ...
            for (const entry of allEntries) {
                ...
                if (fileName === '.DS_Store' || fileName === 'Thumbs.db')
                    continue;
                try {
                    const safePath = (0, security_1.sanitizePath)(entry.entryName);
                    const data = entry.getData();
                    const fileInfo = {
                        entryPath: safePath,
                        fileName: fileName,
                        extension: (0, security_1.getExtension)(fileName),
                        size: data.length,
                        isDirectory: false,
                        data: Buffer.from(data),
                        encoding: (0, security_1.detectEncoding)(Buffer.from(data)),
                    };
            ...
            throw new Error(`ZIP 文件解析失败：${err instanceof Error ? err.message : String(err)}`);
        }
    }
    /**
     * 获取所有文件条目
...
//# sourceMappingURL=zip-processor.js.map