...
export class FileParser {
  ...
  private parseMarkdown(file: ZipFileInfo): ParseResult {
  ...
  /** HTML 解析：去除标签，提取文本 */
  private parseHtml(file: ZipFileInfo): ParseResult {
    const text = this.decodeBuffer(file.data, file.encoding);
  ...
  /** JSON 解析：格式化为可读文本 */
  private parseJson(file: ZipFileInfo): ParseResult {
    const text = this.decodeBuffer(file.data, file.encoding);
    const metadata: Record<string, string> = {};
  ...
  /** XML 解析：去除标签，提取文本 */
  private parseXml(file: ZipFileInfo): ParseResult {
    const text = this.decodeBuffer(file.data, file.encoding);
  ...
  /** CSV 解析：表格格式化 */
  private parseCsv(file: ZipFileInfo): ParseResult {
    const text = this.decodeBuffer(file.data, file.encoding);
  ...
  /** 配置文件解析：保持原始格式 */
  private parseConfig(file: ZipFileInfo): ParseResult {
    const text = this.decodeBuffer(file.data, file.encoding);
    return {
      success: true,
      text: text,
  ...
  /** 纯文本解析 */
  private parseText(file: ZipFileInfo): ParseResult {
    const text = this.decodeBuffer(file.data, file.encoding);
    return {
      success: true,
      text: text,
  ...
  /** 代码文件解析：保留原始代码 */
  private parseCode(file: ZipFileInfo): ParseResult {
    const text = this.decodeBuffer(file.data, file.encoding);
    const metadata: Record<string, string> = {};
   ...
   */
  private parseBinaryDocument(file: ZipFileInfo): ParseResult {
    const metadata: Record<string, string> = {};
  ...
  /** 从二进制数据中提取可读文本片段 */
  private extractTextSnippets(data: Buffer): string {
    // 尝试 UTF-8 解码
    try {
  ...
  private decodeBuffer(buffer: Buffer, encoding: string): string {
    ...
    try {
      switch (encoding) {
        ...
        case 'utf-16be':
          if (buffer.length >= 2) {
            return buffer.subarray(2).toString('utf16be' as BufferEncoding);
          }
          return '';
        case 'gbk':
        default:
