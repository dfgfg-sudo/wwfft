...
1. [环境准备](#一环境准备)
2. [一键启动](#二一键启动)
3. [聊天式界面（chat）](#三聊天式界面chat)
4. [交互式 REPL（interactive）](#四交互式-replinteractive)
5. [单次任务（run）](#五单次任务run)
6. [多模型并行模式（multi）](#六多模型并行模式multi)
7. [工具扫描与连接](#七工具扫描与连接)
8. [报告与上下文](#八报告与上下文)
9. [配置与日志](#九配置与日志)
10. [执行模式详解](#十执行模式详解)
...
`qidi chat` 启动一个聊天式编程助手，参考 Claude Code 的交互体验，支持自然语言对话和命令系统。

### 启动

```bash
qidi chat                              # 默认隐私模式
qidi chat --mode quality               # 高质量模式
qidi chat --provider openai            # 使用 OpenAI
qidi chat -w ./my-project              # 指定工作目录
```

| 选项 | 说明 | 默认值 |
|------|------|--------|
| `-m, --mode` | 执行模式 `privacy` / `quality` | `privacy` |
| `-p, --provider` | 模型提供商 `ollama` / `openai` / `anthropic` | `ollama` 或 `$MODEL_PROVIDER` |
...
直接输入文字即可聊天，AI 会流式逐字回复：

```
  You: 你好，能帮我写代码吗？
  Qidi: 当然可以！我是 Qidi Agent，你的编程助手...
```
...
```
  You: /run 用 Python 写一个待办事项应用
  🚀 开始执行任务: 用 Python 写一个待办事项应用
  ⠋ 正在处理...
  ✅ 任务完成！

  ═══ 任务总结 ═══
  成功率: 100%
  输出目录: ./workspace/task_xxx
  报告 ID: exp_xxx
...
- 聊天记录自动保存到 `~/.qidi/chat_history.json`（最多 50 条）
- 使用 `/history` 查看最近 10 条
- 使用 `/reset` 清空历史
...
### 对比：chat vs interactive

| 特性 | `chat`（聊天式） | `interactive`（REPL） |
|------|-----------------|---------------------|
| 交互方式 | 聊天对话 | 命令行 REPL |
| 命令前缀 | `/` 开头 | 直接输入 |
| 输出方式 | 流式逐字 | 一次性输出 |
| 代码高亮 | 支持 | 支持 |
| 聊天历史 | 自动记忆 | 上下文记忆 |
| 适用场景 | 日常对话、编程辅助 | 专注编程任务 |

---

## 四、交互式 REPL（interactive）
