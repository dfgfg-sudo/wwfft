...
class MimoCodeAdapter extends BaseToolAdapter {
  constructor (options = {}) {
    super({
      name: 'mimo-code',
      displayName: 'Mimo Code',
      description: 'Mimo Code - AI 编程助手',
      command: 'mimo',
      ...options
    });
  }
  ...
  async detect () {
    ...
    try {
      const cmdPath = await this._findCommandInPath('mimo');
      if (cmdPath) {
      ...
      const possiblePaths = [
        path.join(localAppData, 'Programs', 'Mimo Code', 'mimo.exe'),
        path.join(localAppData, 'Mimo Code', 'mimo.exe'),
        path.join(appData, 'Mimo Code', 'mimo.exe'),
        path.join('C:', 'Program Files', 'Mimo Code', 'mimo.exe'),
        path.join('C:', 'Program Files (x86)', 'Mimo Code', 'mimo.exe'),
        path.join(localAppData, 'Programs', 'Mimo', 'mimo.exe'),
        path.join(localAppData, 'Mimo', 'mimo.exe')
      ];
      ...
      if (registryResult) {
        ...
        if (installPathMatch) {
          const exePath = path.join(installPathMatch[1].trim(), 'mimo.exe');
  ...
  async connect (options = {}) {
    ...
    if (!this.detected) {
      throw new Error('Mimo Code 未安装或未找到');
    }

    this.status = 'online';
    return { success: true, message: 'Mimo Code 已就绪' };
  }
  ...
  async execute (task, options = {}) {
    ...
    const outputDir = options.outputDir || `./workspace/mimo-code/${taskId}`;
    ...
    const result = await this._runCommand(this.command, ['run', task], {
  ...
  async collectOutput (taskId) {
    const outputDir = `./workspace/mimo-code/${taskId}`;
    const outputFile = path.join(outputDir, 'output.md');
