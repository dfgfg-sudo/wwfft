...
class ZCodeAdapter extends BaseToolAdapter {
  constructor (options = {}) {
    super({
      name: 'zcode',
      displayName: 'ZCode',
      description: 'ZCode AI Agent - 智能编程助手',
      command: 'zcode',
      ...options
    });
    this.headlessSupported = false;
  }
  ...
  async detect () {
    ...
    try {
      const cmdPath = await this._findCommandInPath('zcode');
      if (cmdPath) {
      ...
      const possiblePaths = [
        ...
        path.join(process.env.APPDATA || '', 'ZCode', 'zcode.exe'),
        path.join('C:', 'Program Files', 'ZCode', 'zcode.exe'),
        path.join('C:', 'Program Files (x86)', 'ZCode', 'zcode.exe'),
        path.join(process.env.HOME || '', 'zcode', 'bin', 'zcode'),
        path.join('/usr', 'local', 'bin', 'zcode')
      ];
  ...
  async checkVersion () {
    try {
      if (process.platform === 'win32' && this.installPath) {
    ...
    try {
      const result = await this._runCommand(this.command, ['--version'], { timeout: 10000, windowsHide: true });
      if (result.success) {
  ...
  async connect (_options = {}) {
    ...
    if (!this.detected) {
      throw new Error('ZCode 未安装或未找到');
    }

    this.status = 'online';
    return { success: true, message: 'ZCode 已就绪' };
  }
  ...
  async execute (task, options = {}) {
    ...
    const outputDir = options.outputDir || `./workspace/zcode/${taskId}`;
    ...
    const result = await this._runCommand(this.command, ['generate', '-t', task], {
