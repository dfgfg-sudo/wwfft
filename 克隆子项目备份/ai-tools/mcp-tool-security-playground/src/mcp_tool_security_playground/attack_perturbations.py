...
Purpose: Prompt-injection perturbation families used for robustness evaluation.
...
MODULE_PURPOSE = 'Prompt-injection perturbation families used for robustness evaluation.'
...
def apply_attack(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def attack_families(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = apply_attack(materialized)
...
def batch_perturb(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
