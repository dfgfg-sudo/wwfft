...
Purpose: Redacted audit-log records for rejected or reviewed tool calls.
...
MODULE_PURPOSE = 'Redacted audit-log records for rejected or reviewed tool calls.'
...
def redact_prompt(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def audit_event(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = redact_prompt(materialized)
...
def summarize_audit_log(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
